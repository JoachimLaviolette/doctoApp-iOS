...
extension ChooseAvailabilityVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if !self.isBookingUpdate! {
            let availabilitiesForDay: [String: [Availability]]!
            if self.availabilitiesPerDay[0] != nil { availabilitiesForDay = self.availabilitiesPerDay[indexPath.row]! }
            else { availabilitiesForDay = self.availabilitiesPerDay[indexPath.row + 1]! }
